...
public class VPayPrint extends PayPrint implements FormPanel, ActionListener, VetoableChangeListener
{
	...
	private void dynInit() throws Exception
	{
		...
		//	Dixon Martinez 05-11-2013
		//	Yamel Zenith 05-11-2013
		// 	Reviewed and updated to add validation to show only pending payments
		MLookup lookupPS = MLookupFactory.get(Env.getCtx(), m_WindowNo, 0, DisplayType.TableDir, Env.getLanguage(Env.getCtx()), "C_PaySelection_ID", 0, false, "EXISTS(SELECT" +
				" 1 FROM C_PaySelection sp INNER JOIN C_PaySelectionCheck csp ON(csp.C_PaySelection_ID = sp.C_PaySelection_ID)WHERE sp.C_PaySelection_ID = C_PaySelection.C_PaySelection_ID AND sp.Processed='Y' AND sp.IsActive='Y' AND csp.C_Payment_ID IS NULL) ");
	...
	private boolean getChecks(String PaymentRule)
	{
		...
		//	Dixon Martinez 04-11-2013
		//	Yamel Zenih	04-11-2013
		//	Set Date Processing Payment Batch
		MPaySelectionCheck m_PaySelectionCheck = new MPaySelectionCheck(Env.getCtx(), m_C_PaySelection_ID,null);
		m_batch.setProcessingDate((Timestamp) m_PaySelectionCheck.get_Value("PayDate"));
		return true;
	}   //  getChecks
